document.addEventListener("DOMContentLoaded", () => {
    // Define language mapping for redirection
    const languageMapping = {
        "EN": "index_EN.html",
        "FR": "index_FR.html",
        "ES": "index_ES.html",
        "PT-BR": "index_PT_BR.html",
    };

    // Get the language selector and dropdown elements
    const languageSelector = document.querySelector('.mil-has-children'); // Updated to use mil-has-children
    const dropdown = languageSelector.querySelector('ul');

    // Ensure the dropdown is hidden when the page loads
    dropdown.style.display = 'none'; // Explicitly hide dropdown on page load
    ...
    // Add event listener for selecting a language
    dropdown.addEventListener('click', (event) => {
        event.stopPropagation(); // Prevent closing the dropdown when selecting a language
        const selectedLanguage = event.target.getAttribute('data-lang');

        if (selectedLanguage && languageMapping[selectedLanguage]) {
            // Update the displayed language in the menu
            const selectedLanguageElement = languageSelector.querySelector('#selected-language');
            if (selectedLanguageElement) {
                selectedLanguageElement.textContent = selectedLanguage; // Update the displayed language
            }

            // Redirect to the selected language page
            window.location.href = languageMapping[selectedLanguage];
        }
    });
});
...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const languageMessage = languageMessages[userLanguage] || languageMessages["default"];

    // Create the message element
    const messageElement = document.createElement("div");
    messageElement.className = "language-message-box";
    messageElement.textContent = languageMessage;

    // Append the message to the language selector
    const languageSelector = document.querySelector(".language-selector");
    languageSelector.appendChild(messageElement);
